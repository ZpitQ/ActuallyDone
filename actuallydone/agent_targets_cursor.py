"""Cursor MCP configuration target."""
...
class CursorTarget(AgentTarget):
    ...
    def config_path(self, options: McpInstallOptions) -> Path:
        return ((options.home_dir / ".cursor" / "mcp.json") if options.location == "global"
                else options.project_root / ".cursor" / "mcp.json")

    def marker_path(self, options: McpInstallOptions) -> Path:
        return options.project_root / ".cursor" / "ACTUALLYDONE.md"
